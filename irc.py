...
from functionality.message_box.msg import place_msg,get_msg

# to do : put a description in wiki command ( with an argument ?? )
...
class irc :
    ...
    def command_response(self,commande,name,channel_message):
        if commande[0] == '!help':
            ...
            wiki_url,description = wiki(' '.join(commande[1::]))
            self.send(wiki_url,channel_message)
            self.send(get_title(wiki_url),channel_message)
            if description != None:
                self.send("[ Description: ] "+description,channel_message)
        elif commande[0] == '!join':
            self.socket.sendall(bytes("JOIN "+commande[1]+"\n", "UTF-8"))

        elif commande[0] == "!msg":
            try:
                if place_msg(name,commande[1],commande[2::]) == True:
                    self.send(('I save your message for ' + commande[1]),name)
                else:
                    self.send('Sry an error was happend',name)
            except:
                self.send("I can't save the message check the !help command",channel_message)
    ...
    def recive_data(self):
            ...
            if line[0].split(' ')[1].strip() == 'JOIN':
                check_box = get_msg(name)
                if check_box != False:
                    for i in check_box:
                        self.send(i.strip(),name)
